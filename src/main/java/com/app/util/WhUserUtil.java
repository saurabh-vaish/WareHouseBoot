...
public class WhUserUtil {
	...
	public void ganeratePie(String path,List<Object[]> data)
	{
		...
		// create dataset from data
			...
			JFreeChart chart = ChartFactory.createPieChart3D("WhUser Type Pie Chart", dataset,true,true,false); // (title,dataset,legend,tooltip,urls)

			// JFreeChart should be converted to image using ChartUtils

			try {

				ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/whUserTypePie.jpg"),chart,450,400); //(outStrem , chart,width,height)
	...
	public void ganerateBar(String path,List<Object[]> data)
	{
		...
		JFreeChart chart =ChartFactory.createBarChart("WhUser Type Bar Chart ", "Order Mode","Count",dataset, PlotOrientation.VERTICAL, true, true, false);
		// arguments - (title, x-axis name, y-axis name,orientation , legend , tooltip, urls)
		
		try {
			ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/whUserTypeBar.jpg"), chart, 450, 400);
		}
		catch(IOException e)
		{
			e.printStackTrace();
